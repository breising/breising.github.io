...
$(document).ready(function() {
    ...
    view.showMainTabs = function() {
        $('.dashBoardButton').show();
        $('.patientButton').show();
        $('.loginTab').css("margin-left", "0");
        $('.loginTab').css("margin-right", "0");
    }

    view.hideMainTabs = function() {
        $('.dashBoardButton').hide();
        $('.patientButton').hide();
        $('.loginTab').css("margin-left", "auto");
        $('.loginTab').css("margin-right", "auto");
    }
    ...
    view.onfocus = function(element) {
        ...
        console.log(element.type + element.defaultValue);

        if (element.defaultValue === "password") {
...
*/
    ...
    view.hideAll = function() {
        ...
        $('.note-contain').hide();
        $('.xray').hide();
        //main page...remember to show them on logout
        $('.contain_part2').hide();
        $('.product').hide();
        $('.toggle-status').hide();
        $('#d-contain').empty();
    ...
    view.newUserFx = function() {
        ...
        view.dataRef.createUser({
            ...
            password: view.phonyPassowrd
                //view.currUser.password
            ...
            if (error) {
                ...
                // create email response for the new patient event
                //
                view.resetPassword(view.currUser.email);
                $('#mySpinner').empty();
                view.spinner.stop();
                var email2 = 'support@orthocure.biz';
                var subject = 'Welcome to OrthoCure!';
                var content = "Thank you " + view.currUser.email + " for registering as an OrthoCure provider. \n\nYour temporary password will be sent to you in a separate email. Please be sure to login and change your password within 24 hours or the password will expire. \n\nOnce you change your password please check out our online training content. Go to the dashboard and select the LEARN tab. You can find everything you need to know about how to operate the OrthoCure system. \n\nAny questions? Our support team is available during North American business hours (east and west coast) at support@orthocure.biz.  \n\nWe look forward to working with you. \n\nwww.orthocure.biz";
                view.sendEmail(view.currUser.email, email2, subject, content);
    ...
    view.resetAllForms = function() {
        ...
        changePassOldPass.type = "text";
        changePassNewPass.type = "text";
    ...
    view.addInitialData = function() {
        ...
        ref.set({
            ...
            if (error) {
                ...
                alert('Success creating new user account! An email was sent to you containing your temporary password that will expire in 24 hours. Please login soon and change your password.');
...
*/
    ...
    view.resetPassword = function(email) {
        ...
        ref.resetPassword({
            ...
            if (error) {
                ...
                alert("A temporary password was sent to your email address:  " + view.currUser.email);
                view.resetAllForms();
            }
        });
    }
    ...
    */
    $('.changePass').click(function() {
        view.hideAll();
        $('#changePassEmail').val(view.currUser.email);
        $('#changePassOldPass').val(view.currUser.password);
        $('#changePass').show();
        $(window).scrollTop($('#changePass').offset().top - 130);
        //view.changePassword(view.currUser.email, view.currUser.password);
    })
    ...
    */
    ...
    view.updateDisplay = function() {
        ...
        if (view.currUser.email === null || view.currUser.email === '' || view.currUser.email === undefined) {
            ...
            $('.displayUser').text(view.currUser.email);
            $('.displayUser').show();
        }
    ...
    view.selectFileNameFirst = function(id) { //before uploading the pick button is hidden...it is turned on when the user selects a file name from the pulldown
        ...
        ref.once('value', function(snapshot) {
                ...
                // adjust the display of the user and patient name, uncenter the logo, show email left and patient name right.
